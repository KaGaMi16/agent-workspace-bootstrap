...
Portable source trees may contain regular files and directories only. Symlinks, sockets, devices, FIFOs, nested `.git`, unreadable files, foreign-owned entries, group/world-writable entries, excessive file counts, and oversized files block automatic migration. The lexical parent chain from the selected home to every registered source is checked before traversal.
